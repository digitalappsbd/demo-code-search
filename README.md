...
- `BAAI/bge-large-en` - a top-performing embedding model that excels at semantic search tasks

## BGE Embeddings Model

As an alternative to the default models, this project also supports using the BAAI/bge-large-en model for generating embeddings. This model is one of the top performers on the MTEB benchmark for retrieval tasks and produces high-quality 1024-dimensional embeddings.

### Generating BGE Embeddings

To generate embeddings using the BGE model:

```shell
# Make sure the script is executable
chmod +x tools/generate_embeddings.sh

# Run the embedding generation script
./tools/generate_embeddings.sh
```

This will:

1. Create or use an existing Python virtual environment
2. Install required dependencies
3. Generate embeddings for all code structures in your structures.json file
4. Save the embeddings to the data/embeddings.json file

### Benefits of BGE Model

The BGE model offers several advantages:

- Higher quality embeddings compared to simple hash-based methods
- Better semantic understanding of code and documentation
- Support for instruction prefixes to enhance retrieval performance
- State-of-the-art results on benchmarks for retrieval tasks

When searching, the system will automatically use these high-quality embeddings if they're available, falling back to simpler methods if needed.
